  ...
  return (
    <div className="space-y-8 animate-fade-in max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          <h1 className="text-3xl font-black tracking-tight text-gray-900 dark:text-white">{t('dash.title')}</h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1 flex items-center gap-2">
            {t('dash.desc').replace('{name}', tenant.name)}
          </p>
        </div>
        ...
        <div className="flex items-center gap-3 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 px-4 py-2 rounded-xl shadow-sm">
          <div className={clsx("flex flex-col", isRtl ? "items-start" : "items-end")}>
            <span className="text-[10px] font-bold uppercase tracking-wider text-gray-400">{t('dash.currency.active')}</span>
          ...
          {isPendingCurrency && (
            <>
              ...
              <div className={clsx("flex flex-col", isRtl ? "items-end" : "items-start")}>
                <span className="text-[10px] font-bold uppercase tracking-wider text-amber-500 flex items-center gap-1">
                  <ShieldAlert size={10} /> {t('dash.currency.pending')}
                </span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-dark-panel border border-gray-100 dark:border-gray-800 p-6 rounded-2xl shadow-xl shadow-gray-200/40 dark:shadow-none relative overflow-hidden group">
          <div className={clsx("absolute -top-6 text-blue-50 dark:text-blue-900/20 group-hover:scale-110 transition-transform duration-500", isRtl ? "-left-6" : "-right-6")}>
            <CircleDollarSign size={120} />
          </div>
          <div className="relative z-10">
            <p className="text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">{t('dash.revenue')}</p>
            ...
            <p className="text-xs font-semibold text-emerald-500 flex items-center gap-1">
              <ArrowUpRight size={14} /> {t('dash.revenue.trend')}
            </p>
          </div>
        </div>

        <div className="bg-white dark:bg-dark-panel border border-gray-100 dark:border-gray-800 p-6 rounded-2xl shadow-xl shadow-gray-200/40 dark:shadow-none relative overflow-hidden group">
          <div className={clsx("absolute -top-6 text-emerald-50 dark:text-emerald-900/20 group-hover:scale-110 transition-transform duration-500", isRtl ? "-left-6" : "-right-6")}>
            <PackageSearch size={120} />
          </div>
          <div className="relative z-10">
            <p className="text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">{t('dash.stock')}</p>
            <h3 className="text-4xl font-black text-gray-900 dark:text-white mb-2">{totalVolume}</h3>
            <p className="text-xs font-semibold text-gray-500">{t('dash.stock.desc').replace('{count}', inventory.length.toString())}</p>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-dark-panel border border-rose-100 dark:border-rose-900/30 p-6 rounded-2xl shadow-xl shadow-rose-200/20 dark:shadow-none relative overflow-hidden group">
           <div className="relative z-10">
            <p className="text-sm font-bold text-rose-500 dark:text-rose-400 uppercase tracking-wider mb-2">{t('dash.alert')}</p>
            <h3 className="text-4xl font-black text-rose-600 dark:text-rose-500 mb-2">{lowStockCount}</h3>
            <p className="text-xs font-semibold text-rose-500/70">{t('dash.alert.desc')}</p>
          </div>
        </div>
